...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
      for j in range(num_test):
          dists[i][j] = torch.sum((x_train[i].view(1, -1) - x_test[j].view(1, -1))**2)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  # Replace "pass" statement with your code
  for i in range(num_train):
      dists[i, :] = torch.sum((x_train[i].view(1, -1) - x_test.view(num_test, -1))**2, axis = 1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  x_squares = torch.sum(x_train.view(num_train, -1)**2, axis = 1).view(num_train, 1)
  y_squares = torch.sum(x_test.view(num_test, -1)**2, axis = 1).view(1, num_test)
#   print(x_squares.shape, y_squares.shape)
  dists =  x_squares + y_squares - 2* x_train.view(num_train, -1).matmul(x_test.view(num_test, -1).t())
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  for i in range(num_test):
      top_res = y_train[torch.topk(dists[:, i], k, largest = False)[1]]
      y_pred[i] = torch.mode(top_res , 0)[0]
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for i in range(len(k_choices)):
      ...
      for j in range(num_folds):
          list_train = list(range(num_folds))
          list_train.remove(j)
          ...
          for k in range(num_folds):
              if k == j:
                  continue
              x_train_data.append(x_train_folds[k])
              y_train_data.append(y_train_folds[k])
          x_train_data = torch.cat(x_train_data)
          y_train_data = torch.cat(y_train_data)
          
          classifier = KnnClassifier(x_train_data, y_train_data)
          acc_lis.append(classifier.check_accuracy(x_val, y_val, k=k_choices[i]))
      k_to_accuracies[i] = acc_lis
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
